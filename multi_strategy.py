...
class BondMomentumStrategy:
    ...
    diag_bonds 实测(2026-07-08):夏普 1.5、回撤 -4.4%,与 CTA 相关性 -0.115
...
class RiskParityMulti:
    ...
    diag_riskparity 日频诊断:静态全样本σ(有前视)risk-parity 夏普 1.91 vs 等权 1.29、
    Δ=+0.60、P=0.001(显著)——但这是日频再平衡 + 前视σ的上界。月频落地(项目实际调仓频率)
    经 engine.py multi 实测:risk-parity 夏普 1.45 vs 等权 1.29、Δ=+0.16、P=0.182 **不显著**
    (月频再平衡滞后吃掉优势)。按"不显著就不切"的方法论,默认保持等权,本类留作可选
    (更看重回撤/Sortino 或改日频调仓时才显现价值)。权重稳定在 CTA~15% / 国债~85%。
