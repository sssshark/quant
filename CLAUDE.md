...
- **Price-source separation in live (信号价 ≠ 下单价)**: `decide_targets` and the limit-up/down test must use the tushare **前复权历史序列** (`recent`); the order-sizing/account price (`build_orders`'s `price`, `read_account`/`reconcile`) must use the **QMT realtime tick** (`_qmt_realtime_prices`), falling back to tushare's last close. Rationale: `_qfq_from_pre_close` keeps the latest value un-adjusted, so after close the two coincide; intraday only the QMT price reflects the real fill. Do **not** feed QMT realtime into the signal path (复权断崖) nor tushare-close into order sizing (昨收 vs 盘中日内差). Limit-up/down test stays on `recent[-2]/recent[-1]` 环比 — it needs the *series*, never a single realtime tick.
