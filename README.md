...
与 CTA 相关性 −0.06~−0.11（股债跷跷板），是唯一通过"三道关"的独立 alpha 来源。
...
| 配置 | 夏普 | 年化 | 回撤 | 说明 |
|---|---|---|---|---|
| CTA 动量选股+风控（`real` 模式） | 1.20 | 15.1% | −20.2% | 默认回测口径，仅作策略展示 |
| CTA 等权全池+风控（`hold_all=True`） | ≈1.0 | — | — | **实盘推荐**：选股无可靠 alpha（PBO 0.67），等权更稳 |
| 国债时序动量（独立） | 1.5 | — | −4.4% | 第二策略 |
| CTA + 国债 50/50（等权组合） | 1.29 | — | — | > 单 CTA，分散有效 |
| CTA + 国债（risk-parity） | 1.91 | — | — | Δ=+0.60，P=0.001；权重国债约 85%（低波，绝对收益温和） |
...
~/miniconda3/bin/python engine.py            # 默认 real：等权+风控 vs 基准，出图
...
- 行情走 **tushare HTTP**（默认代理 `https://fastapic.stockai888.top`，可 `TUSHARE_API` 环境变量覆盖）。
